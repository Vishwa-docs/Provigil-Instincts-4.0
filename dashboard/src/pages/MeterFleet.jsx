...
function HealthBar({ score }) {
 ...
 pct >= 85 ? 'bg-green-500' :
 pct >= 70 ? 'bg-blue-500' :
 pct >= 50 ? 'bg-orange-500' : 'bg-red-500';  return (
...
export default function MeterFleet() {
 ...
 const [statusFilter, setStatusFilter] = useState(searchParams.get('status') || '');  useEffect(() => {
 async function load() {
 setLoading(true);
 try {
 ...
 load();
 }, [statusFilter]);  const filteredMeters = meters
 .filter((m) => {
 ...
 <div data-tour="meter-fleet-page">
 <PageHeader
 title="Meter Fleet"
 ...
 <Link
 to={`/meters/${meter.id}`}
 className="text-sm font-medium text-gray-700 hover:text-[#0071E3] transition-colors"
 >
 {meter.name}
 </Link>
 ...
 <span className="text-sm text-orange-500">{formatIssueLabel(meter.suspected_issue)}</span>
 ) : (

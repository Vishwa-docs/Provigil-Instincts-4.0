...
function AlertIcon({ severity }) {
 ...
 default:
 return <Info className="w-5 h-5 text-[#0071E3]"/>;
 }
}
...
function AlertDetailPanel({ alert, onClose }) {
 ...
 const [woResult, setWoResult] = useState(null);  async function handleSummarize() {
 ...
 className="text-[10px] px-2 py-1 rounded-lg bg-purple-50 text-purple-600 border border-purple-500/20 hover:bg-purple-50 transition-colors"
 >
 ...
 <span className="text-purple-600 mt-0.5">•</span> {r}
 </li>
 ))}
 </ul>
 </div>
 ...
 </div>  {/* Generate Work Order */}
 <div className="p-3 rounded-xl bg-gray-50 border border-gray-200">
 <div className="flex items-center justify-between mb-2">
 <div className="flex items-center gap-1.5">
 <ClipboardList className="w-3.5 h-3.5 text-orange-500"/>
 <span className="text-xs font-medium text-orange-500">Work Order</span>
 </div>
 {!woResult && (
 <button
 onClick={handleGenerateWorkOrder}
 disabled={generatingWO}
 className="text-[10px] px-2 py-1 rounded-lg bg-orange-50 text-orange-500 border border-orange-200 hover:bg-orange-50 transition-colors"
 >
 ...
 {woResult ? (
 woResult.error ? (
 <p className="text-xs text-red-500">{woResult.error}</p>
 ) : (
 <div className="text-xs">
 <p className="text-gray-600 font-medium">{woResult.title || 'Work order created'}</p>
 <p className="text-[10px] text-gray-400 mt-1">{woResult.description?.slice(0, 120)}</p>
 {woResult.work_order_id && (
 <Link to="/workorders"className="text-[10px] text-[#0071E3] hover:text-blue-600 mt-2 inline-block">
 View in Work Orders →
 </Link>
 )}
 </div>
 ...
 <Link to={`/meters/${alert.meter_id}`} className="px-2.5 py-1 rounded-lg bg-blue-50 text-[#0071E3] border border-blue-200 hover:bg-blue-50 transition-colors">
...
export default function Alerts() {
 ...
 const [expandedAlertId, setExpandedAlertId] = useState(null);  async function loadAlerts() {
 setLoading(true);
 try {
 ...
 loadAlerts();
 }, [severityFilter]);  function handleSeverityFilter(nextFilter) {
 ...
 <p className="text-xl font-bold text-gray-900">{stats.by_severity?.[sev] || 0}</p>
 ...
 <p className="text-xl font-bold text-gray-900">{stats.total_unacknowledged || 0}</p>
 ...
 <Link to={`/meters/${alert.meter_id}`} className="text-xs text-[#0071E3] hover:text-blue-600 font-mono">
 {alert.meter_id}
 </Link>
 <span className="text-xs text-gray-500">•</span>
